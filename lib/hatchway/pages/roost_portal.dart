...
(() => {
  const root = window;
  if (root.__eraInsetKeeper) return;
  root.__eraInsetKeeper = true;
...
''');
  ...
  void _installZoomLock() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__eraZoomLock) return;
  window.__eraZoomLock = true;
  const lockViewport = () => {
    const host = document.head || document.documentElement;
    if (!host) return;
...
''');
  ...
  void _installTapPolish() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__eraTapPolish) return;
  window.__eraTapPolish = true;
...
''');
  ...
  void _installKeyboardLift() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__eraInputLift) return;
  window.__eraInputLift = true;
...
''');
  ...
  void _installFocusScaleGuard() {
    ...
    _controller.runJavaScript(r'''
(() => {
  if (window.__eraFocusScale) return;
  window.__eraFocusScale = true;
  const style = document.createElement('style');
  style.textContent =
...
''');
  ...
  void _installInlinePlayback() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__eraInlineMedia) return;
  window.__eraInlineMedia = true;
  const awaken = (video) => {
