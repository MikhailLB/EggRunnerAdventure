...
void eraTrace(String Function() message) {
  assert(() {
    debugPrint(message());
    return true;
  }());
...
class FlightAttribution {
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      eraTrace(() => '[ERA.FLIGHT] initialization failed: $error');
      _completeEmpty();
    }
  }
  ...
  Future<void> _acceptInstall(dynamic raw) async {
    try {
      ...
      eraTrace(
        () => '[ERA.FLIGHT] conversion status=$status '
      ...
      eraTrace(() => '[ERA.FLIGHT] conversion parse error: $error');
      _install = <String, dynamic>{};
    } finally {
  ...
  Future<Map<String, dynamic>> compose({
    ...
    eraTrace(() => '[ERA.FLIGHT] payload ${jsonEncode(body)}');
    return body;
  }
